...
# When the camera view shifts slightly, these offsets move the saved zones together
# with the video image. This is useful for CCTV/video clips where the view moves once.
AUTO_ZONE_TRACKING = True
ZONE_TRACKING_MAX_STEP = 80.0
ZONE_TRACKING_MIN_POINTS = 12
zone_offset_x = 0.0
zone_offset_y = 0.0
zone_track_prev_gray = None
...
def shift_zones(zones, dx, dy, frame_shape=None):
    """Return zones shifted by the current camera-motion offset."""
    shifted = []
    h = w = None
    ...
    for zone in zones:
        ...
        for x, y in zone:
            ...
            if w is not None and h is not None:
                nx = max(0, min(w - 1, nx))
                ny = max(0, min(h - 1, ny))
            new_zone.append((nx, ny))
        shifted.append(new_zone)
    return shifted


def get_active_danger_zones(frame_shape=None):
    return shift_zones(danger_zones, zone_offset_x, zone_offset_y, frame_shape)


def get_active_ignore_zones(frame_shape=None):
    return shift_zones(ignore_zones, zone_offset_x, zone_offset_y, frame_shape)


def update_zone_motion_tracking(frame):
    """Estimate global camera translation and move zones with the image.

    This uses sparse optical flow. It works best when the video has a small pan,
    shake, or one-time shift. It is not meant for large zoom/rotation changes.
    """
    global zone_track_prev_gray, zone_offset_x, zone_offset_y

    if not AUTO_ZONE_TRACKING or frame is None:
        return

    gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)

    if zone_track_prev_gray is None:
        zone_track_prev_gray = gray
        return

    # Track stable corners from the previous frame to the current frame.
    prev_pts = cv.goodFeaturesToTrack(
        zone_track_prev_gray,
        maxCorners=300,
        qualityLevel=0.01,
        minDistance=12,
        blockSize=7
    ...
    if prev_pts is None or len(prev_pts) < ZONE_TRACKING_MIN_POINTS:
        zone_track_prev_gray = gray
        return
    ...
    if len(good_prev) < ZONE_TRACKING_MIN_POINTS:
        zone_track_prev_gray = gray
        return
    ...
    # Reject abnormal jumps caused by detection/tracking failure.
    if abs(dx) <= ZONE_TRACKING_MAX_STEP and abs(dy) <= ZONE_TRACKING_MAX_STEP:
        zone_offset_x += dx
        zone_offset_y += dy
...
def person_touches_danger_zone(det):
    ...
    return any(is_in_any_zone(pt, get_active_danger_zones()) for pt in check_points)
...
def run_detection(model, frame):
    ...
    for result in results:
        for box in result.boxes:
            ...
            if is_in_any_zone(center, get_active_ignore_zones(frame.shape)):
                continue
            detections.append({
                "class": class_name,
                "conf": conf,
...
def render_frame(frame, detections, update_warning_state=True):
    ...
    active_ignore_zones = get_active_ignore_zones(display.shape)
    active_danger_zones = get_active_danger_zones(display.shape)

    for i, zone in enumerate(active_ignore_zones):
        draw_polygon(display, zone, (120, 120, 120), f"Ignore Zone {i + 1}")

    for i, zone in enumerate(active_danger_zones):
    ...
    if warning:
        for i, zone in enumerate(get_active_danger_zones(display.shape)):
...
def split_shortcut_label(text):
    """Split a button label into shortcut key and action text.

    Examples:
    "[SPACE] PAUSE VIEW" -> ("SPACE", "PAUSE VIEW")
...
def draw_keycap_text(img, key_text, action_text, rect, scale=0.55):
    """Draw shortcut key as a separate keycap box for better visibility."""
    x1, y1, x2, y2 = rect
    h = y2 - y1
    ...
    if not key_text:
        size, _ = cv.getTextSize(action_text, cv.FONT_HERSHEY_SIMPLEX, scale, 2)
        tx = x1 + (x2 - x1 - size[0]) // 2
        ty = y1 + (h + size[1]) // 2
        draw_ui_text(img, action_text, (tx, ty), scale)
        return

    # Wider keycap for long keys such as SPACE and ESC.
    if len(key_text) >= 5:
        key_w = 96
        key_scale = 0.52
    elif len(key_text) >= 3:
    ...
    key_x1 = x1 + 64
    key_y1 = y1 + 10
    key_x2 = min(key_x1 + key_w, x2 - 135)
    key_y2 = y2 - 10

    # Keycap background and border.
    cv.rectangle(img, (key_x1, key_y1), (key_x2, key_y2), (35, 35, 35), -1)
    cv.rectangle(img, (key_x1, key_y1), (key_x2, key_y2), (245, 245, 245), 2)

    # Key text centered inside the keycap.
    key_size, _ = cv.getTextSize(key_text, cv.FONT_HERSHEY_SIMPLEX, key_scale, 2)
    kx = key_x1 + (key_x2 - key_x1 - key_size[0]) // 2
    ky = key_y1 + (key_y2 - key_y1 + key_size[1]) // 2
    draw_ui_text(img, key_text, (kx, ky), key_scale, (255, 255, 255), 2)

    # Action text after the keycap.
    action_x = key_x2 + 16
    action_size, _ = cv.getTextSize(action_text, cv.FONT_HERSHEY_SIMPLEX, scale, 2)
    action_y = y1 + (h + action_size[1]) // 2
    draw_ui_text(img, action_text, (action_x, action_y), scale, (255, 255, 255), 2)
...
def draw_button(img, text, rect, color, active=False, scale=0.55):
    ...
    # Draw shortcut keys as separate keycap boxes.
    # This improves visibility for keys like SPACE, ESC, [, and ].
    key_text, action_text = split_shortcut_label(text)
    draw_keycap_text(img, key_text, action_text, rect, scale)
...
def zone_manager():
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if in_rect(x, y, clear_ignore_button):
            ...
            return
        # IMPORTANT:
        # Check DELETE buttons before list-area selection.
        # Otherwise a click on DELETE can be swallowed by the larger list rectangle.
...
BUTTONS = [
    ...
    ("[[] SEGMENT -", (355, 215, 635, 265), "record_time_down"),
    ("]] SEGMENT +", (355, 280, 635, 330), "record_time_up"),
]
...
def draw_control_panel(reader_connected=True, reader_error="", stale_seconds=0.0):
    ...
    draw_ui_text(panel, f"Detect FPS: {detection_fps:.1f}", (365, 585), 0.48, (220, 220, 220), 1)
    draw_ui_text(panel, f"Record: {record_state}", (365, 610), 0.48, (0, 0, 255) if record_video else (220, 220, 220), 1)
    draw_ui_text(panel, f"Segment: {segment_label}", (365, 635), 0.45, (0, 255, 255), 1)
    ...
    draw_ui_text(panel, f"Current: {current_file}", (365, 660), 0.40, (220, 220, 220), 1)
    draw_ui_text(panel, f"Capture: {capture_file}", (365, 683), 0.40, (220, 220, 220), 1)
...
if __name__ == "__main__":
    ...
    while running:
        ...
        if frame is not None:
            update_zone_motion_tracking(frame)
